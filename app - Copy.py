...
class Application:
    ...
    def __init__(self):
        ...
        for i in ascii_uppercase:
          self.ct[i] = 0
        print("Loaded model from disk")#loading models end
         ...
         #   cv2image = cv2.cvtColor(cv2image, cv2.COLOR_BGR2RGBA)
          #  self.current_image = Image.fromarray(cv2image)
           # imgtk = ImageTk.PhotoImage(image=self.current_image)
            ...
            ret, res = cv2.threshold(th3, 120, 255, cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)
            self.predict(res)
    ...
    def predict(self,test_image):#to prdict the sign
